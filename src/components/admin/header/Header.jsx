    ...
    return (
        <li className="m-2 relative">
            <NavLink 
                to={link} 
                className={({ isActive }) => 
                    isActive ? "text-background-white bg-[#35363A] afterStyle w-36 block rounded-xl text-center p-2" : "text-background-white w-36 block rounded-full text-center p-2"
                }
            >
                {icon}
                {title}
    ...
    const headerLinks = [
        {
            ...
            icon: <i className="fi fi-sr-address-card flex items-center justify-center"></i>

        },
        {
            title: "کد های تخفیف",
            link: "/admin/offer-codes",
            icon: <i className="fi fi-sr-binary-circle-check flex items-center justify-center"></i>
        },
        {
            title: "سفارشات",
            link: "/admin/orders",
            ...
            <nav className="flex items-center justify-between">
                ...
                <ul className="flex w-full justify-evenly items-center p-2">
                    {headerLinks.map((headerLink, index) => 
                        <HeaderLink key={index} title={headerLink.title} link={headerLink.link} icon={headerLink.icon} />
                    )}
                </ul>
            </nav>
            <NormalBtn title={`خروج`} path={`/`} />
        </header>
    );
};
